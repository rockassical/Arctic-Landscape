    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void VertexFunction(Attributes attribs, out Interpolators varyings)
            {
	            ...
	            varyings.uv_ws = position_ws.xz + _WorldSpaceCameraPos.xz;
            ...
            void FragmentFunction(Interpolators varyings, out half4 outColor : SV_Target)
            {
	            ...
	            half depth = Linear01Depth(SampleCameraDepth(uv_ss), _ZBufferParams) * _ProjectionParams.z - (varyings.position_ss.w - 1);
	            half depthMask = saturate(depth - _Depth);

	            // Calculating Refracted Depth
	            half refDepth = Linear01Depth(SampleCameraDepth(uv_ss + normal.xz * _Refraction), _ZBufferParams) * _ProjectionParams.z - (varyings.position_ss.w - 1);
	            half refMask = saturate(depth - _Depth);

	            // Shallow-Deep Coloring
	            half3 waterColor = lerp(_ColorShallow, _ColorDeep, refMask);

	            // Specular Coloring
	            half3 halfVector = normalize(viewDir);
	            ...
	            half fresnel = Fresnel(normal, viewDir, _AmbientFresnel);
